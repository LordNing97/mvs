...
function initFlterMenu() {
    ...
    $(".esh-filter-sidebar .close").click(function () {
        $(".esh-filter-sidebar").removeClass("open");
        $("#btnFilter").removeClass("expand").addClass("collapse").text('筛选')
    })
    flterMenuView();
...
function showException(data) {
    growlAlert.danger("操作请求失败，请稍后再试！");
}
...
function abnormalFunction(value) {
    if (value == 0) {
        return "原厂选件";
    } else if (value == 1) {
        return "拆机件";
...
function goodOrBadFunction(value) {
    if (value == 0) {
        return "原件";
    } else if (value == 1) {
        return "质保期内DOA";
...
function sparePartStateFunction(value){
    if(value == 0){
        return "采购待入库";
    }else if(value == 1){
        return "在库";
